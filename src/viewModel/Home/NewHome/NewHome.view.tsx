...
import { ActivityIndicator, FlatList, Text, TextInput, TouchableOpacity, View, Modal, ScrollView, Dimensions } from "react-native";
  ...
  return (
    <SafeAreaView className="flex-1 bg-background-primary">
      ...
      <View className="flex-row justify-between bg-gray-100 p-1 mx-5 mt-4 rounded-xl border border-gray-200/50">
        ...
        >
          <Text className={`text-xs font-bold ${searchMode === "explorar" ? "text-[#092D5D]" : "text-gray-600"}`}>
            Explorar
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          <Text className={`text-xs font-bold ${searchMode === "buscar" ? "text-[#092D5D]" : "text-gray-600"}`}>
            Buscar Próximos
          </Text>
        </TouchableOpacity>
      </View>
        ...
        <View className="px-5 mt-4 mb-4">
          <View className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm gap-3">
            ...
            <View className="flex-row items-center justify-between mt-1">
              <Text className="text-gray-600 text-xs font-semibold">Raio de busca:</Text>
              <View className="flex-row gap-1.5">
                {[5, 15, 25, 50].map((r) => (
                  ...
                  >
                    <Text className={`text-[10px] font-bold ${radius === r ? "text-[#092D5D]" : "text-gray-600"}`}>
                      {r} km
                    </Text>
                  </TouchableOpacity>
                ))}
      ...
      {searchMode === "explorar" && favoriteCompanies && favoriteCompanies.length > 0 && (
        <View className="mb-6">
          <View className="flex-row justify-between items-center px-5 mb-3">
            <Text className="text-[#12294A] text-sm font-bold">Favoritos</Text>
            <TouchableOpacity 
              onPress={() => safePush("/(private)/(tabs)/(client-tabs)/(menu)/favorites")}
              activeOpacity={0.7}
            >
              <Text className="text-[#CBA35D] text-xs font-semibold">Ver todos</Text>
            </TouchableOpacity>
          </View>
          <FlatList
            ...
            renderItem={({ item }) => {
              const images = item.imagesUrl
                ? item.imagesUrl.split(",").map((x) => x.trim()).filter(Boolean)
                : [];
              ...
              return (
                ...
                >
                  <View className="w-full h-14 rounded-xl overflow-hidden bg-gray-50">
                    {firstImage ? (
                      <ExpoImage
                  ...
                  <View className="mt-1.5 px-1 flex-row justify-between items-center">
                    <Text className="text-[#12294A] font-bold text-[10px] flex-1 mr-1" numberOfLines={1}>
                      {item.name}
                    </Text>
      ...
      {!!access_token && user?.roles?.some(r => r.authority === "ROLE_CLIENT") && user?.firstLogin !== false && (() => {
        const handleNext = () => {
          if (slideIndex < 2) {
        ...
        const slides = [
          {
            ...
            description: "Marque seus horários a qualquer hora do dia ou da noite, em qualquer lugar, sem precisar ligar ou enviar mensagens.",
          ...
          },
          {
            ...
            description: "Descubra os melhores profissionais e estabelecimentos pertinho de você, com mapa interativo e buscas customizadas.",
            icon: "map-outline",
          ...
          },
          {
            ...
            description: "Visualize seus próximos agendamentos, cancele quando necessário e acompanhe todo seu histórico em tempo real.",
            icon: "time-outline",
        ...
        return (
          <Modal transparent animationType="fade" visible={true}>
            <View className="flex-1 bg-[#092D5D]/40 justify-end">
              ...
              <View 
                style={{ flex: 1, backgroundColor: slides[slideIndex].bgColor }} 
                className="justify-between pt-12 pb-6 px-6"
              >
                {/* Header (Pular / Skip) */}
                <View className="flex-row justify-between items-center w-full">
                  <View className="flex-row items-center gap-1.5 bg-black/10 px-3.5 py-1.5 rounded-full">
                    ...
                    <Text className="text-[#092D5D] font-bold text-xs">Novo por aqui?</Text>
                  </View>
                  <TouchableOpacity onPress={handleDismissOnboarding} activeOpacity={0.7} className="bg-[#092D5D]/10 px-4 py-2 rounded-full">
                ...
                <View className="items-center justify-center flex-1 my-4">
                  <View className="bg-white p-8 rounded-full shadow-md items-center justify-center">
                    <Ionicons name={slides[slideIndex].icon as any} size={84} color={slides[slideIndex].iconColor} />
                  </View>
                </View>
                <View />
              </View>
              ...
              {/* Bottom White Card */}
              <View className="bg-white rounded-t-[40px] px-8 pt-8 pb-10 shadow-2xl h-[370px] justify-between">
                  ...
                  <View className="flex-row gap-1.5">
                    {slides.map((_, idx) => (
                      <View
                        ...
                        style={{
                          backgroundColor: idx === slideIndex ? "#092D5D" : "#cbd5e1",
                        }}
                      />
                    ))}
                  </View>
